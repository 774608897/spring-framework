...
/**
 * Delegate for resolving constructors and factory methods. Performs constructor
 * resolution through argument matching.
 *
...
class ConstructorResolver {
	...
	/**
	 * Create a new ConstructorResolver for the given factory and instantiation
	 * strategy.
	 * 
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		ArgumentsHolder argsHolderToUse = null;// 构造参数持有者
		...
		try {
			...
			if (System.getSecurityManager() != null) {
				return AccessController.doPrivileged((PrivilegedAction<Object>) () -> strategy.instantiate(mbd,
						beanName, this.beanFactory, constructorToUse, argsToUse),
	 ...
	 * {@link RootBeanDefinition#getResolvedFactoryMethod()} can be checked for the
	 * result.
	 * 
	...
	/**
	 * Retrieve all candidate methods for the given class, considering the
	 * {@link RootBeanDefinition#isNonPublicAccessAllowed()} flag. Called as the
	 * starting point for factory method determination.
	 */
	 ...
	 * if the bean definition parameter specifies a class, rather than a
	 * "factory-bean", or an instance variable on a factory object itself configured
	 * using Dependency Injection.
	 * <p>
	 * Implementation requires iterating over the static or instance methods with
	 * the name specified in the RootBeanDefinition (the method may be overloaded)
	 * and trying to match with the parameters. We don't have the types attached to
	 * constructor args, so trial and error is the only way to go here. The
	 * explicitArgs array may contain argument values passed in programmatically via
	 * the corresponding getBean method.
	 * 
	 * @param beanName     the name of the bean
	 * @param mbd          the merged bean definition for the bean
	 * @param explicitArgs argument values passed in programmatically via the
				...
				if (paramTypes.length >= minNrOfArgs) {
					...
					if (explicitArgs != null) {
						...
						try {
							...
							argsHolder = createArgumentArray(beanName, mbd, resolvedValues, bw, paramTypes, paramNames,
									candidate, autowiring, candidates.length == 1);
						} catch (UnsatisfiedDependencyException ex) {
							// 若发生 UnsatisfiedDependencyException 异常，添加到 causes 中。
							if (logger.isTraceEnabled()) {
		...
		try {
			if (System.getSecurityManager() != null) {
				return AccessController.doPrivileged(
						(PrivilegedAction<Object>) () -> this.beanFactory.getInstantiationStrategy().instantiate(mbd,
								beanName, this.beanFactory, factoryBean, factoryMethod, args),
						this.beanFactory.getAccessControlContext());
			} else {
	...
	/**
	 * Create an array of arguments to invoke a constructor or factory method, given
	 * the resolved constructor argument values.
	 */
					...
					try {
						...
						throw new UnsatisfiedDependencyException(mbd.getResourceDescription(), beanName,
								...
								"Could not convert argument value of type ["
										+ ObjectUtils.nullSafeClassName(valueHolder.getValue()) + "] to required type ["
										+ paramType.getName() + "]: " + ex.getMessage());
					}
	...
	/**
	 * Template method for resolving the specified argument which is supposed to be
	 * autowired.
	 */
	@Nullable
